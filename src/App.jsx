...
// ─── PRICERS ────────────────────────────────────────────────────────────────
function binomAmer(S,K,T,r,σ,N,put){
  const dt=T/N,u=Math.exp(σ*Math.sqrt(dt)),d=1/u,p=(Math.exp(r*dt)-d)/(u-d),disc=Math.exp(-r*dt);
...
function barrierMC(S0,K,T,r,σ,H,isDown,isOut,put,Z){
  const nS=Z[0].length,dt=T/nS,disc=Math.exp(-r*T),mu=(r-.5*σ*σ)*dt,v=σ*Math.sqrt(dt);
  let tot=0;
...
function digital(S,K,T,r,σ,isCall,isCash){
  if(T<=0)return isCall?(S>K?(isCash?1:S):0):(S<K?(isCash?1:S):0);
  const sq=Math.sqrt(T),d1=(Math.log(S/K)+(r+.5*σ*σ)*T)/(σ*sq),d2=d1-σ*sq,e=Math.exp(-r*T);
  return isCash?(isCall?e*Phi(d2):e*Phi(-d2)):(isCall?S*Phi(d1):S*Phi(-d1));
}
function lookbackMC(S0,K,T,r,σ,isFloat,put,Z){
  const nS=Z[0].length,dt=T/nS,disc=Math.exp(-r*T),mu=(r-.5*σ*σ)*dt,v=σ*Math.sqrt(dt);
  let tot=0;
...
function shoutMC(S0,K,T,r,σ,put,Z){
  const nS=Z[0].length,dt=T/nS,disc=Math.exp(-r*T),mu=(r-.5*σ*σ)*dt,v=σ*Math.sqrt(dt);
  let tot=0;
  for(const path of Z){let S=S0,floor=0;for(let j=0;j<nS;j++){S*=Math.exp(mu+v*path[j]);const tRem=T-(j+1)*dt;if(floor===0&&(put?S<K:S>K)&&tRem>dt){const atm=bs(S,S,tRem,r,σ,put);if((put?K-S:S-K)>=atm)floor=put?K-S:S-K;}}tot+=Math.max(put?Math.max(K-S,0):Math.max(S-K,0),floor);}
  return disc*tot/Z.length;
}
function asianMC(S0,K,T,r,σ,put,Z){
  const nS=Z[0].length,dt=T/nS,disc=Math.exp(-r*T),mu=(r-.5*σ*σ)*dt,v=σ*Math.sqrt(dt);
  let tot=0;
...
function basketMC(S1,S2,K,T,r,σ1,σ2,rho,w1,put,Z1,Z2c){
  const nS=Z1[0].length,dt=T/nS,disc=Math.exp(-r*T),mu1=(r-.5*σ1*σ1)*dt,v1=σ1*Math.sqrt(dt),mu2=(r-.5*σ2*σ2)*dt,v2=σ2*Math.sqrt(dt);
  let tot=0;
...
function rainbowMC(S1,S2,K,T,r,σ1,σ2,rho,type,Z1,Z2c){
  const nS=Z1[0].length,dt=T/nS,disc=Math.exp(-r*T),mu1=(r-.5*σ1*σ1)*dt,v1=σ1*Math.sqrt(dt),mu2=(r-.5*σ2*σ2)*dt,v2=σ2*Math.sqrt(dt);
  let tot=0;
...
function chooserMC(S0,K,T,t0,r,σ,Z){
  const nS0=Math.round(Z[0].length*t0/T),dt=T/Z[0].length,mu=(r-.5*σ*σ)*dt,v=σ*Math.sqrt(dt);
  let tot=0;
  for(const path of Z){let S=S0;for(let j=0;j<nS0;j++)S*=Math.exp(mu+v*path[j]);tot+=Math.max(bs(S,K,T-t0,r,σ,false),bs(S,K,T-t0,r,σ,true));}
  return Math.exp(-r*t0)*tot/Z.length;
}
function cliquetMC(S0,T,r,σ,cap,floor,nPeriods,Z){
  const nS=Z[0].length,dt=T/nS,mu=(r-.5*σ*σ)*dt,v=σ*Math.sqrt(dt),disc=Math.exp(-r*T),spp=Math.floor(nS/nPeriods);
  let tot=0;
...
function powerMC(S0,K,T,r,σ,n,put,Z){
  const nS=Z[0].length,dt=T/nS,disc=Math.exp(-r*T),mu=(r-.5*σ*σ)*dt,v=σ*Math.sqrt(dt);
  let tot=0;
...
function fwdStart(S0,alpha,T,t0,r,σ,put){return S0*bs(1,alpha,T-t0,r,σ,put);}


function binomBermudan(S,K,T,r,σ,N,put,nDates){
  const dt=T/N,u=Math.exp(σ*Math.sqrt(dt)),d=1/u,p=(Math.exp(r*dt)-d)/(u-d),disc=Math.exp(-r*dt);
  let V=Array.from({length:N+1},(_,i)=>Math.max(put?K-S*Math.pow(u,N-2*i):S*Math.pow(u,N-2*i)-K,0));
...
function parisianMC(S0,K,T,r,σ,H,isDown,isOut,window,put,Z){
  const nS=Z[0].length,dt=T/nS,disc=Math.exp(-r*T),mu=(r-.5*σ*σ)*dt,v=σ*Math.sqrt(dt);
...
function quantoBS(S,K,T,r,σ,σFX,rho,put){
  if(T<=0||S<=0)return Math.max(put?K-S:S-K,0);
  const q_adj=rho*σ*σFX; 
  const sq=Math.sqrt(T),d1=(Math.log(S/K)+(r-q_adj+.5*σ*σ)*T)/(σ*sq),d2=d1-σ*sq,e=Math.exp(-r*T),eq=Math.exp(-q_adj*T);
  return put?K*e*Phi(-d2)-S*eq*Phi(-d1):S*eq*Phi(d1)-K*e*Phi(d2);
}

// Full autocall MC returning {price, callProbs, kiProb, surviveProb, payoffs}
function autocallFull(S0,barrierPct,kiPct,couponRate,nObs,T,r,σ,nPaths,seed=42){
  const rand=lcg(seed),dt=T/nObs,mu=(r-.5*σ*σ)*dt,v=σ*Math.sqrt(dt);
...
function autocallMC(S0,barrierPct,kiPct,couponRate,nObs,T,r,σ,nPaths,seed=42){return autocallFull(S0,barrierPct,kiPct,couponRate,nObs,T,r,σ,nPaths,seed).price;}


// MC Paths Generator for Recharts
function genMCPathsData(S0, T, r, q, σ, numPaths=12, numSteps=60, seed=42) {
  ...
  avgPath.forEach((sum, step) => {
    data[step].avg = sum / numPaths;
  });

  return { data, pathKeys };
}

// MULTI-ASSET MC Paths Generator
function genMultiMCPathsData(S1, S2, T, r, q, σ1, σ2, rho, numPaths=20, numSteps=60, seed=42) {
  const rand = lcg(seed);
  const dt = T / numSteps;
  const mu1 = (r - (q || 0) - 0.5 * σ1 * σ1) * dt;
  const v1 = σ1 * Math.sqrt(dt);
  const mu2 = (r - (q || 0) - 0.5 * σ2 * σ2) * dt;
  const v2 = σ2 * Math.sqrt(dt);
  ...
  for (let p = 0; p < numPaths; p++) {
    ...
    for (let step = 1; step <= numSteps; step++) {
      const z1 = bm(rand(), rand());
      const z2 = bm(rand(), rand());
      const z2c = rho * z1 + sqRho * z2; 
      
      A *= Math.exp(mu1 + v1 * z1);
      B *= Math.exp(mu2 + v2 * z2c);
...
function Sl({label,value,min,max,step,unit,onChange,accent="#00e5ff"}){
  ...
  React.useEffect(() => {
    if (parseFloat(value) !== parseFloat(localVal)) {
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
        ...
        <div style={{display:"flex", alignItems:"center"}}>
          <input 
            ...
            style={{
              background: C.bg, border: `1px solid ${C.border}`, color: C.bright, fontSize: 10,
              fontWeight: 700, width: 46, padding: "3px 4px", borderRadius: 4, textAlign: "right",
              outline: `1px solid transparent`, fontFamily: "'IBM Plex Mono',monospace", marginRight: 6, transition: "border 0.2s"
            }}
    ...
    <div onClick={onToggle} style={{background:C.panel,borderRadius:10,padding:"11px 10px 5px",border:`1px solid ${active===gk?color+"77":C.border}`,cursor:"pointer",opacity:dimmed?.15:1,transition:"all .2s",transform:active===gk?"scale(1.012)":"scale(1)"}}>
      ...
      <ResponsiveContainer width="100%" height={250}>
        <LineChart data={d} margin={{top:2,right:4,left:0,bottom:0}}>
          ...
          <XAxis dataKey="S" type="number" domain={['dataMin', 'dataMax']} tick={{fill:C.muted,fontSize:8}} tickLine={false} axisLine={{stroke:C.border}}/>             <YAxis tick={{fill:C.muted,fontSize:8}} tickLine={false} axisLine={false} width={46} tickFormatter={v=>Math.abs(v)<0.001?v.toExponential(1):Math.abs(v)<1?v.toFixed(3):v.toFixed(1)}/>
          ...
          <Line type="monotone" dataKey={gk} stroke={color} strokeWidth={2} dot={false} activeDot={{r:3,fill:color,stroke:C.panel,strokeWidth:2}} isAnimationActive={false}/>        </LineChart>
      </ResponsiveContainer>
    </div>
  );
}
...
function plinearStrategy(p, n=70){
  const minK = Math.min(...p.legs.map(l=>l.K)) || p.S0;
  const maxK = Math.max(...p.legs.map(l=>l.K)) || p.S0;
  const Smin = Math.max(0, minK * 0.5);
  const Smax = maxK * 1.5;
...
const OPTS=[
  {id:"european",name:"European",  sub:"Black-Scholes Model",           color:"#00e5ff",cat:"Vanilla",      emoji:"📐",closedForm:true,
   dp:{S0:100,K:100,T:1,r:.05,sigma:.20,put:false},
   pf:(S,p)=>bs(S,p.K,p.T,p.r,p.q||0,p.sigma,p.put),kRef:p=>p.K,payoff:p=>plinear(p.K,p.put)},

  {id:"american",name:"American",  sub:"CRR Tree (150 steps)",   color:"#60a5fa",cat:"Vanilla",      emoji:"🌳",
   dp:{S0:100,K:100,T:1,r:.05,sigma:.20,put:false},
   pf:(S,p)=>binomAmer(S,p.K,p.T,p.r,p.sigma,150,p.put),kRef:p=>p.K,payoff:p=>plinear(p.K,p.put)},

  {id:"barrier",  name:"Barrier",   sub:"Knock-Out / Knock-In",          color:"#f97316",cat:"Exotic",     emoji:"🚧",
   dp:{S0:100,K:100,T:1,r:.05,sigma:.20,H:80,isDown:true,isOut:true,put:false},
   pf:(S,p,Z1)=>barrierMC(S,p.K,p.T,p.r,p.sigma,p.H,p.isDown,p.isOut,p.put,Z1||genZ(2500,60,99)),
  ...
  {id:"digital",  name:"Digital",   sub:"Cash / Asset-or-Nothing", color:"#e879f9",cat:"Exotic",     emoji:"⚡",closedForm:true,
   dp:{S0:100,K:100,T:1,r:.05,sigma:.20,isCall:true,isCash:true},
   pf:(S,p)=>digital(S,p.K,p.T,p.r,p.sigma,p.isCall,p.isCash),kRef:p=>p.K,
  ...
  {id:"lookback", name:"Lookback",   sub:"Floating/Fixed Strike",    color:"#fb7185",cat:"Exotic",     emoji:"👁",
   dp:{S0:100,K:100,T:1,r:.05,sigma:.20,isFloat:true,put:false},
   pf:(S,p,Z1)=>lookbackMC(S,p.K,p.T,p.r,p.sigma,p.isFloat,p.put,Z1||genZ(2500,60,55)),
  ...
  {id:"shout",    name:"Shout",      sub:"Optimal Lock-in",         color:"#4ade80",cat:"Exotic",     emoji:"📢",
   dp:{S0:100,K:100,T:1,r:.05,sigma:.20,put:false},
   pf:(S,p,Z1)=>shoutMC(S,p.K,p.T,p.r,p.sigma,p.put,Z1||genZ(2500,60,33)),kRef:p=>p.K,
  ...
  {id:"asian",    name:"Asian",  sub:"Arithmetic Average",    color:"#a78bfa",cat:"Path-Dep",  emoji:"📊",
   dp:{S0:100,K:100,T:1,r:.05,sigma:.20,put:false},
   pf:(S,p,Z1)=>asianMC(S,p.K,p.T,p.r,p.sigma,p.put,Z1||genZ(2500,60,11)),kRef:p=>p.K,
  ...
  {id:"basket",   name:"Basket",     sub:"2 Correlated Assets",       color:"#fbbf24",cat:"Multi-Asset",  emoji:"🧺",
   dp:{S0:100,S2:100,K:100,T:1,r:.05,sigma:.20,sigma2:.25,rho:.5,w1:.5,put:false},
   pf:(S,p,Z1,Z2)=>{const Zc=corrZ(Z1||genZ(2500,60,22),Z2||genZ(2500,60,33),p.rho||.5);return basketMC(S,p.S2||100,p.K,p.T,p.r,p.sigma,p.sigma2||.25,p.rho||.5,p.w1||.5,p.put,Z1||genZ(2500,60,22),Zc);},
  ...
  {id:"rainbow",  name:"Rainbow",    sub:"Best-of / Worst-of",      color:"#f43f5e",cat:"Multi-Asset",  emoji:"🌈",
   dp:{S0:100,S2:100,K:100,T:1,r:.05,sigma:.20,sigma2:.25,rho:.5,type:"best-call"},
   pf:(S,p,Z1,Z2)=>{const Zc=corrZ(Z1||genZ(2500,60,22),Z2||genZ(2500,60,33),p.rho||.5);return rainbowMC(S,p.S2||100,p.K,p.T,p.r,p.sigma,p.sigma2||.25,p.rho||.5,p.type||"best-call",Z1||genZ(2500,60,22),Zc);},
  ...
  {id:"chooser",  name:"Chooser",    sub:"Call/Put Choice at t₀",     color:"#06b6d4",cat:"Other",       emoji:"🎯",
   dp:{S0:100,K:100,T:1,t0:.5,r:.05,sigma:.20},
   pf:(S,p,Z1)=>chooserMC(S,p.K,p.T,p.t0||.5,p.r,p.sigma,Z1||genZ(2500,60,77)),kRef:p=>p.K,
   payoff:p=>Array.from({length:60},(_,i)=>{const S=35+(210-35)*i/59;const Tr=p.T-(p.t0||.5);return{S:+S.toFixed(1),payoff:Math.max(bs(S,p.K,Tr,p.r,p.sigma,false),bs(S,p.K,Tr,p.r,p.sigma,true))};})},

  {id:"cliquet",  name:"Cliquet",    sub:"Periodic Returns Cap/Floor",color:"#84cc16",cat:"Other",   emoji:"🔗",
   dp:{S0:100,T:2,r:.05,sigma:.20,cap:.10,floor:-.05,nPeriods:4},
   pf:(S,p,Z1)=>cliquetMC(S,p.T,p.r,p.sigma,p.cap||.1,p.floor||-.05,p.nPeriods||4,Z1||genZ(2500,60,55)),
  ...
  {id:"power",    name:"Power",      sub:"Payoff = S^n − K",         color:"#f59e0b",cat:"Other",       emoji:"⚡",
   dp:{S0:100,K:10000,T:1,r:.05,sigma:.20,n:2,put:false},
   pf:(S,p,Z1)=>powerMC(S,p.K,p.T,p.r,p.sigma,p.n||2,p.put,Z1||genZ(2500,60,88)),
  ...
  {id:"exchange", name:"Exchange",    sub:"max(S₂−S₁, 0) Margrabe",  color:"#38bdf8",cat:"Other",       emoji:"🔄",closedForm:true,
   dp:{S0:100,S2:110,T:1,r:.05,sigma:.20,sigma2:.25,rho:.3},
  ...
  {id:"fwdstart", name:"Forward Start",sub:"Deferred Strike α·S(t₀)",color:"#34d399",cat:"Other",       emoji:"⏩",closedForm:true,
   dp:{S0:100,alpha:1.0,T:2,t0:.5,r:.05,sigma:.20,put:false},
   pf:(S,p)=>fwdStart(S,p.alpha||1,p.T,p.t0||.5,p.r,p.sigma,p.put),
   kRef:p=>null,payoff:p=>Array.from({length:60},(_,i)=>{const S=40+(200-40)*i/59;return{S:+S.toFixed(1),payoff:bs(1,p.alpha||1,p.T-(p.t0||.5),p.r,p.sigma,p.put)*S};})},

  {id:"autocall", name:"Autocall",   sub:"Structured Product MC",     color:"#fb923c",cat:"Structured",    emoji:"🏛",
   dp:{S0:100,barrierPct:1,kiPct:.6,couponRate:.10,nObs:4,T:4,r:.03,sigma:.20},
   pf:(S,p)=>autocallMC(S,p.barrierPct,p.kiPct,p.couponRate,p.nObs,p.T,p.r,p.sigma,2000),
   ...
   {id:"bermudan", name:"Bermudan", sub:"Discrete Binomial Tree", color:"#10b981", cat:"Vanilla", emoji:"🌴",
   dp:{S0:100,K:100,T:1,r:.05,sigma:.20,put:false,nDates:4},
   pf:(S,p)=>binomBermudan(S,p.K,p.T,p.r,p.sigma,150,p.put,p.nDates||4),kRef:p=>p.K,payoff:p=>plinear(p.K,p.put)},

  {id:"parisian", name:"Parisian", sub:"Excursion Window", color:"#c084fc", cat:"Exotic", emoji:"🗼",
   dp:{S0:100,K:100,T:1,r:.05,sigma:.20,H:80,isDown:true,isOut:true,window:10,put:false},
   pf:(S,p,Z1)=>parisianMC(S,p.K,p.T,p.r,p.sigma,p.H,p.isDown,p.isOut,p.window||10,p.put,Z1||genZ(2500,60,99)),
   kRef:p=>p.K,refs:p=>[{x:p.H,rc:p.isOut?"#c084fcaa":"#22d3eeaa",rl:p.isOut?"KO":"KI"}],
   payoff:p=>Array.from({length:70},(_,i)=>{const St=30+(230-30)*i/69;const hit=p.isDown?St<=p.H:St>=p.H;return{S:+St.toFixed(1),payoff:(p.isOut?!hit:hit)?Math.max(p.put?p.K-St:St-p.K,0):0};})},

  {id:"quanto", name:"Quanto", sub:"FX Hedged Option", color:"#ec4899", cat:"Multi-Asset", emoji:"💱", closedForm:true,
   dp:{S0:100,K:100,T:1,r:.05,sigma:.20,sigma2:.15,rho:.3,put:false},
   pf:(S,p)=>quantoBS(S,p.K,p.T,p.r,p.sigma,p.sigma2||.15,p.rho||.3,p.put),kRef:p=>p.K,payoff:p=>plinear(p.K,p.put)},

   {id:"strategy", name:"Builder", sub:"Options Portfolio", color:"#fb7185", cat:"Strategies", emoji:"🧩", closedForm:true,
   ...
   {id:"volsurface", name:"3D Surface", sub:"Implied Volatility", color:"#e879f9", cat:"Market", emoji:"🌐", isTool:true}
  
];
// ─── SHARED PARAM SLIDERS per option type ───────────────────────────────────
function ParamSliders({opt,p,sp}){
  ...
  const hasS0=p.S0!==undefined,hasK=p.K!==undefined&&["european","american","bermudan","barrier","parisian","digital","lookback","shout","asian","basket","rainbow","chooser","exchange","fwdstart","quanto"].includes(id);
  const hasPut=p.put!==undefined;
  return(
    <div>
      {hasS0&&<Sl label="Spot S₀" value={p.S0} min={50} max={200} step={1} unit="" accent={color} onChange={v=>sp("S0",v)}/>}
      {id==="autocall"&&<><Sl label="Spot S₀" value={p.S0} min={80} max={130} step={1} unit="" accent={color} onChange={v=>sp("S0",v)}/><Sl label="Barrier AC" value={(p.barrierPct*100).toFixed(0)} min={80} max={120} step={1} unit="%" accent={color} onChange={v=>sp("barrierPct",v/100)}/><Sl label="Knock-In" value={(p.kiPct*100).toFixed(0)} min={40} max={90} step={1} unit="%" accent="#ef4444" onChange={v=>sp("kiPct",v/100)}/><Sl label="Coupon/obs" value={(p.couponRate*100).toFixed(1)} min={1} max={25} step={.5} unit="%" accent="#34d399" onChange={v=>sp("couponRate",v/100)}/><Sl label="Observations" value={p.nObs} min={1} max={8} step={1} unit="" accent={color} onChange={v=>sp("nObs",v)}/></>}
      ...
      {id!=="autocall"&&<><Sl label="Maturity T" value={p.T} min={.1} max={id==="fwdstart"?5:3} step={.1} unit=" y" accent={color} onChange={v=>sp("T",v)}/><Sl label="Rate r" value={(p.r*100).toFixed(1)} min={0} max={15} step={.1} unit="%" accent={color} onChange={v=>sp("r",v/100)}/><Sl label="Yield q" value={((p.q||0)*100).toFixed(1)} min={0} max={15} step={.1} unit="%" accent="#10b981" onChange={v=>sp("q",v/100)}/><Sl label="Vol σ" value={(p.sigma*100).toFixed(0)} min={5} max={80} step={1} unit="%" accent={color} onChange={v=>sp("sigma",v/100)}/></>}
      {id==="autocall"&&<><Sl label="Maturity T" value={p.T} min={1} max={8} step={.5} unit=" y" accent={color} onChange={v=>sp("T",v)}/><Sl label="Rate r" value={(p.r*100).toFixed(1)} min={0} max={10} step={.1} unit="%" accent="#a78bfa" onChange={v=>sp("r",v/100)}/><Sl label="Yield q" value={((p.q||0)*100).toFixed(1)} min={0} max={10} step={.1} unit="%" accent="#10b981" onChange={v=>sp("q",v/100)}/><Sl label="Vol σ" value={(p.sigma*100).toFixed(0)} min={5} max={60} step={1} unit="%" accent="#a78bfa" onChange={v=>sp("sigma",v/100)}/></>}
...
function AutocallExtraCharts({p,color}){
  const mc=React.useMemo(()=>autocallFull(p.S0,p.barrierPct,p.kiPct,p.couponRate,p.nObs,p.T,p.r,p.sigma,8000),[p.S0,p.barrierPct,p.kiPct,p.couponRate,p.nObs,p.T,p.r,p.sigma]);
  ...
  const sensData=Array.from({length:20},(_,i)=>{const σ=(10+i*3)/100;return{sigma:+(σ*100).toFixed(0),price:+autocallMC(p.S0,p.barrierPct,p.kiPct,p.couponRate,p.nObs,p.T,p.r,σ,2000).toFixed(2)};});
        ...
        <ResponsiveContainer width="100%" height={250}>
          <BarChart data={callProbData} margin={{top:4,right:8,left:0,bottom:0}}>
            ...
            <Tooltip 
              ...
              contentStyle={{background:C.panel,border:`1px solid ${color}44`,borderRadius:7,fontFamily:"'IBM Plex Mono',monospace",fontSize:10}}
        ...
        <ResponsiveContainer width="100%" height={250}>
          <BarChart data={histData} margin={{top:4,right:8,left:0,bottom:0}}>
            ...
            <Bar dataKey="pct" radius={[4,4,0,0]}>
              {histData.map((e,i)=>{
                const mid=Number(e.range.split("–")[0])+5;
                const c=mid<p.kiPct*100?"#ef4444":mid>100?color:"#34d399";
                return <Cell key={i} fill={c} fillOpacity={1}/>;
              })}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
        <div style={{display:"flex",gap:14,marginTop:6,fontSize:8,color:C.muted}}>
          <span><span style={{color:"#ef4444"}}>■</span> Loss (KI)</span>
          <span><span style={{color:"#34d399"}}>■</span> Capital Safe</span>
          <span><span style={{color}}> ■</span> Recall + Coupon</span>
        </div>
      </ChartBox>
...
function AsianExtraCharts({p,color}){
  ...
  const compData=Array.from({length:20},(_,i)=>{const σ=(5+i*4)/100;return{sigma:+(σ*100).toFixed(0),asian:+asianMC(p.S0,p.K,p.T,p.r,σ,p.put,Z).toFixed(3),euro:+bs(p.S0,p.K,p.T,p.r,σ,p.put).toFixed(3)};});
...
function BarrierExtraCharts({p,color}){
  ...
  const survData=Array.from({length:20},(_,i)=>{const H=p.isDown?40+i*5:110+i*5;const price=barrierMC(p.S0,p.K,p.T,p.r,p.sigma,H,p.isDown,true,p.put,Z);return{H:+H.toFixed(0),price:+price.toFixed(3)};});
...
function MCPathsChart({ S0, T, r, q, sigma, color, title = "Monte Carlo Paths Visualization" }) {
  const { data, pathKeys } = React.useMemo(() => {
    return genMCPathsData(S0, T, r, q, sigma, 100, 60);
  }, [S0, T, r, q, sigma]);
    ...
    <ChartBox title={title} accent={color}>
      <ResponsiveContainer width="100%" height={500}>
        <LineChart data={data} margin={{ top: 2, right: 8, left: 0, bottom: 0 }}>
          ...
          <Tooltip 
            cursor={{ stroke: "#ffffff22", strokeWidth: 1, strokeDasharray: "4 4" }}
            content={({ active, payload, label }) => {
              if (active && payload && payload.length) {
                const avgData = payload.find(p => p.dataKey === 'avg');
                if (!avgData) return null;
                return (
                  <div style={{ background: C.panel, border: `1px solid ${color}44`, padding: "8px 12px", borderRadius: 7, fontFamily: "'IBM Plex Mono',monospace", fontSize: 10 }}>
                    <div style={{ color: C.muted, marginBottom: 4 }}>Step {label}</div>
                    <div style={{ color: "#ffffffaa", fontWeight: 700 }}>Average : {avgData.value.toFixed(2)}</div>
                  </div>
                );
              }
              return null;
          ...
          {pathKeys.map((key, i) => (
            <Line key={key} type="monotone" dataKey={key} stroke={color} strokeWidth={0.8} strokeOpacity={0.4} dot={false} activeDot={false} isAnimationActive={false} />
          ))}

          <Line type="monotone" dataKey="avg" stroke="#ffffffaa" strokeWidth={2.5} dot={false} name="Average" isAnimationActive={false} />
        </LineChart>
      </ResponsiveContainer>
      <div style={{ fontSize: 8, color: C.muted, marginTop: 4 }}>100 simulated paths. The average converges to the theoretical value.</div>
    </ChartBox>
  );
}

function MultiMCPathsChart({ S1, S2, T, r, q, sigma1, sigma2, rho, color1, color2="#a78bfa", title="Correlated Multi-Asset Paths" }) {
  const { data, pathKeys1, pathKeys2 } = React.useMemo(() => {
    return genMultiMCPathsData(S1, S2, T, r, q, sigma1, sigma2, rho, 20, 60);
  }, [S1, S2, T, r, q, sigma1, sigma2, rho]);
  ...
  return (
    <ChartBox title={title} accent={color1}>
        ...
        <LineChart data={data} margin={{ top: 2, right: 8, left: 0, bottom: 0 }}>
          ...
          <Tooltip 
            ...
            content={({ active, label }) => {
              if (active) {
                return (
                  <div style={{ background: C.panel, border: `1px solid ${C.border}`, padding: "8px 12px", borderRadius: 7, fontFamily: "'IBM Plex Mono',monospace", fontSize: 10 }}>
                    <div style={{ color: C.muted }}>Step {label}</div>
                    <div style={{ color: color1, marginTop: 4 }}>Asset 1 (Spot S₁)</div>
                    <div style={{ color: color2 }}>Asset 2 (Spot S₂)</div>
                  </div>
                );
              }
              return null;
          ...
          {pathKeys1.map((key) => (
            <Line key={key} type="monotone" dataKey={key} stroke={color1} strokeWidth={1} strokeOpacity={0.6} dot={false} activeDot={false} isAnimationActive={false} />
          ))}
          {pathKeys2.map((key) => (
            <Line key={key} type="monotone" dataKey={key} stroke={color2} strokeWidth={1} strokeOpacity={0.6} dot={false} activeDot={false} isAnimationActive={false} />
          ))}
        </LineChart>
      </ResponsiveContainer>
      <div style={{ fontSize: 8, color: C.muted, marginTop: 6, display: "flex", gap: 14 }}>
        ...
        <span>Correlation ρ = {(rho*100).toFixed(0)}%</span>
      </div>
    </ChartBox>
  );
}
...
function PricerPage({opt,p,sp}){
  ...
  const price=opt.pf(cp.S0,cp,Z1,Z2);
  ...
  if(opt.id==="european"||opt.id==="american"||opt.id==="strategy"){
    ...
    if (opt.id === "strategy") {
      cp.legs.forEach(l => {
        const legG = bsGreeks(cp.S0, l.K, cp.T, cp.r, cp.q||0, l.sigma, l.type==='put');
        const mult = l.dir * l.qty;
        g.delta += legG.delta * mult; g.gamma += legG.gamma * mult; g.theta += legG.theta * mult;
        g.vega += legG.vega * mult; g.rho += legG.rho * mult; g.vanna += legG.vanna * mult; g.vomma += legG.vomma * mult;
      });
    } else {
      g = bsGreeks(cp.S0, cp.K, cp.T, cp.r, cp.q||0, cp.sigma, cp.put);
    }

    kpis=[{l:"Delta Δ",v:g.delta.toFixed(4),c:"#00e5ff"},{l:"Gamma Γ",v:g.gamma.toFixed(5),c:"#ff6b35"},{l:"Theta Θ",v:g.theta.toFixed(4),c:"#a78bfa"},{l:"Vega ν",v:g.vega.toFixed(4),c:"#34d399"},{l:"Rho ρ",v:g.rho.toFixed(4),c:"#fbbf24"},{l:"Vanna",v:g.vanna.toFixed(4),c:"#f472b6"},{l:"Vomma",v:g.vomma.toFixed(4),c:"#4ade80"}];
    
    if(opt.id==="american"){
      const eu=bs(cp.S0,cp.K,cp.T,cp.r,cp.q||0,cp.sigma,cp.put);
      kpis=[{l:"American",v:price.toFixed(4),c:color},{l:"European",v:eu.toFixed(4),c:C.muted},{l:"Early Exer. Prem.",v:Math.max(price-eu,0).toFixed(4),c:"#fb923c"},...kpis.slice(0,4)];
    }
  }
  ...
  return(
    <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
          ...
          {!isFast && (
            <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,marginTop:4}}>
              <button onClick={run} disabled={busy}
                style={{width:"100%",padding:"9px 0",background:"transparent",border:`1px solid ${busy?"#f59e0b":color}`,color:busy?"#f59e0b":color,borderRadius:7,cursor:busy?"not-allowed":"pointer",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,letterSpacing:2,textTransform:"uppercase",transition:"all .2s"}}>
      ...
      <div style={{flex:1,minWidth:260,display:"flex",flexDirection:"column",gap:12, opacity: busy ? 0.4 : 1, transition: "opacity 0.2s"}}>
        <Pnl accent={color}>
          <Lbl>Theoretical Value</Lbl>
          <div style={{display:"flex",alignItems:"baseline",gap:10}}>
            <div style={{fontSize:46,fontWeight:700,color,fontFamily:"'Courier New',monospace",lineHeight:1,letterSpacing:1}}>{price!=null?price.toFixed(3):"-"}</div>
            {opt.id==="autocall"&&<span style={{fontSize:16,color:C.muted}}>% of Notional</span>}
          </div>
        </Pnl>
        {kpis.length>0&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(90px,1fr))",gap:8}}>{kpis.map(k=><Kpi key={k.l} label={k.l} value={k.v} color={k.c}/>)}</div>}
        
        <Pnl accent={color}>
          <Lbl>Price (t=0) vs Payoff (T)</Lbl>
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={combinedData} margin={{top:2,right:8,left:0,bottom:0}}>
              ...
              <Tooltip 
                contentStyle={{background:C.panel,border:`1px solid ${color}44`,borderRadius:7,fontFamily:"'IBM Plex Mono',monospace",fontSize:10}} 
                formatter={(v, name) => [v.toFixed(3), name === "price" ? "Current Price (t=0)" : "Payoff at Expiry"]} 
                labelFormatter={l=>`S = ${Number(l).toFixed(1)}`}
              />
              ...
              <ReferenceLine x={cp.S0} stroke="#ffffff44" strokeDasharray="4 4" label={{value:"Current Spot",fill:C.muted,fontSize:8,position:"insideTopLeft"}}/>
              
              <Line type="monotone" dataKey="payoff" stroke="#64748b" strokeWidth={2} strokeDasharray="4 4" dot={false} isAnimationActive={false} />            
          ...
          <div style={{display:"flex",gap:16,marginTop:12,fontSize:9,color:C.muted,justifyContent:"center", letterSpacing: 1}}>
            <span><span style={{color:"#64748b", fontWeight: 700}}>---</span> PAYOFF AT EXPIRY</span>
            <span><span style={{color:color, fontWeight: 700}}>—</span> CURRENT PRICE (t=0)</span>
          </div>
        </Pnl>
...
// ─── GREEKS PAGE (with params sidebar) ──────────────────────────────────────
function GreeksPage({opt,p,sp}){
  const isFast = opt.closedForm || opt.id === "american" || opt.id === "bermudan";
  ...
  const run=React.useCallback(()=>{
    setBusy(true);
    setTimeout(()=>{setData(computeGreekGrid(opt.pf,p));setBusy(false);},10);
  },[opt,p]);
    ...
    <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
      <div style={{flex:"0 0 200px"}}>
        <Pnl style={{position:"sticky",top:72}} accent={opt.color}>
          ...
          <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,marginTop:4}}>
            ...
            {isFast ? (
              <div style={{width:"100%",padding:"9px 0",background:"transparent",border:`1px solid ${opt.color}`,color:opt.color,borderRadius:7,fontFamily:"'IBM Plex Mono',monospace",fontSize:10,letterSpacing:2,textTransform:"uppercase",textAlign:"center"}}>
                ⚡ Real-Time
              </div>
            ) : (
              <button onClick={run} disabled={busy}
                style={{width:"100%",padding:"9px 0",background:"transparent",border:`1px solid ${busy?"#f59e0b":opt.color}`,color:busy?"#f59e0b":opt.color,borderRadius:7,cursor:busy?"not-allowed":"pointer",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,letterSpacing:2,textTransform:"uppercase",transition:"all .2s"}}>
                {busy?"⏳ Computing...":"▶ Recalculate"}
              </button>
            )}
            
            <div style={{marginTop:8,fontSize:8,color:C.muted,textAlign:"center",lineHeight:1.5}}>
              24 pts · {isFast ? "Closed-Form" : "2,000 MC Paths"}<br/>Central Finite Differences
            </div>
          </div>
        </Pnl>
      </div>
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{ opacity: busy && !isFast ? 0.4 : 1, transition: "opacity 0.2s" }}>
          {data && (
            <>
              <GreeksGrid data={data} refs={refs} order={order}/>
              {order==="2" && (
...
// ─── 3D VOLATILITY SURFACE ENGINE ────────────────────────────────────────
function generate3DVolSurface(S0, baseVol, skewFactor, smileFactor) {
  const strikes = [];
  const maturities = [];
  const zData = []; 

  for (let k = S0 * 0.5; k <= S0 * 1.5; k += S0 * 0.05) strikes.push(k.toFixed(0));
  for (let t = 0.1; t <= 2.0; t += 0.1) maturities.push(t.toFixed(1));

  for (let i = 0; i < maturities.length; i++) {
    const T = Number(maturities[i]);
    const zRow = [];
    for (let j = 0; j < strikes.length; j++) {
      const K = Number(strikes[j]);
      const moneyness = Math.log(K / S0);
      
      const skew = skewFactor / Math.sqrt(T);
      const smile = smileFactor / Math.sqrt(T);
      let vol = baseVol + (skew * moneyness) + (smile * moneyness * moneyness);
      
      zRow.push(Math.max(0.01, vol) * 100);
    }
    zData.push(zRow);
  }
...
function VolSurfacePage({ color }) {
  ...
  const { strikes, maturities, zData } = React.useMemo(() => {
    return generate3DVolSurface(S0, baseVol, skewFactor, smileFactor);
  }, [S0, baseVol, skewFactor, smileFactor]);
    ...
    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
      <div style={{ flex: "0 0 200px" }}>
        <Pnl style={{ position: "sticky", top: 72 }} accent={color}>
          ...
          <div style={{ marginTop: 16, fontSize: 9, color: C.muted, lineHeight: 1.6 }}>
            Drag the chart to rotate the 3D surface.
          </div>
        </Pnl>
      </div>
          ...
          <div style={{ width: '100%', height: 500 }}>
            <Plot
              data={[{
                ...
                y: maturities,
                type: 'surface',
                colorscale: [[0, '#00e5ff'], [0.5, '#a78bfa'], [1, '#f472b6']],
                showscale: false
              }]}
              layout={{
                autosize: true,
                ...
                font: { color: C.muted, family: '"IBM Plex Mono", monospace', size: 10 },
                scene: {
                  xaxis: { title: 'Strike K', gridcolor: C.border, zerolinecolor: C.border, showbackground: false },
                  yaxis: { title: 'Maturity T (Years)', gridcolor: C.border, zerolinecolor: C.border, showbackground: false },
                  zaxis: { title: 'Implied Volatility (%)', gridcolor: C.border, zerolinecolor: C.border, showbackground: false },
...
function OptionPage({opt}){
  ...
  const color=opt.color;
  if (opt.isTool && opt.id === "volsurface") {
    return <VolSurfacePage color={color} />;
  }
  return(
    <div>
        ...
        <div style={{display:"flex",background:C.bg,borderRadius:8,border:`1px solid ${C.border}`,overflow:"hidden"}}>
          {[{v:"pricer",l:"💰 Pricer"},{v:"greeks",l:"Δ Greeks f(S)"}].map(o=>(
            <button key={o.v} onClick={()=>setView(o.v)}
              style={{padding:"9px 18px",border:"none",cursor:"pointer",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,letterSpacing:2,textTransform:"uppercase",
                background:view===o.v?color+"22":"transparent",color:view===o.v?color:C.muted,
                borderBottom:view===o.v?`2px solid ${color}`:"2px solid transparent",transition:"all .2s"}}>
              {o.l}
            </button>
          ))}
        </div>
      ...
      {view==="pricer"&&<PricerPage opt={opt} p={params} sp={sp}/>}
      {view==="greeks"&&<GreeksPage opt={opt} p={params} sp={sp}/>}
    </div>
  );
}
        ...
        <div key={cat} style={{marginBottom:14}}>
          ...
          {items.map(t=>(
            <button key={t.id} onClick={()=>onSel(t.id)}
              style={{width:"100%",textAlign:"left",padding:"7px 8px",borderRadius:7,border:"none",background:sel===t.id?"#1a2d45":"transparent",borderLeft:sel===t.id?`2px solid ${t.color}`:"2px solid transparent",cursor:"pointer",fontFamily:"'IBM Plex Mono',monospace",marginBottom:1,transition:"all .15s"}}>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                <span style={{fontSize:13}}>{t.emoji}</span>
                <div><div style={{fontSize:10,fontWeight:700,color:sel===t.id?t.color:"#7a90a8"}}>{t.name}</div><div style={{fontSize:8,color:"#3d5268",marginTop:1}}>{t.sub}</div></div>
              </div>
            </button>
          ))}
        </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'IBM Plex Mono',monospace",boxSizing:"border-box"}}>
      ...
      <div style={{background:"linear-gradient(180deg,#070f1f 0%,#030712 100%)",borderBottom:`1px solid ${C.border}`,padding:"24px 28px 0"}}>
        <div style={{maxWidth:1480,margin:"0 auto"}}>
          <div style={{textAlign:"center",paddingBottom:18}}>
            ...
            <h1 style={{margin:"0 0 10px",fontFamily:"'Courier New',monospace",fontSize:"clamp(26px,4vw,44px)",fontWeight:700,letterSpacing:4,lineHeight:1,
              background:"linear-gradient(110deg,#d4e4f7 0%,#00e5ff 40%,#f59e0b 75%,#fb923c 100%)",
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"center",paddingBottom:0}}>
              {OPTS.map(t=>(
                <button key={t.id} onClick={()=>setSel(t.id)}
                  style={{padding:"6px 13px",borderRadius:"8px 8px 0 0",border:`1px solid ${sel===t.id?t.color:C.border}`,borderBottom:"none",
                    background:sel===t.id?t.color+"22":C.panel,
                    color:sel===t.id?t.color:C.muted,cursor:"pointer",fontSize:10,letterSpacing:.5,
                    fontFamily:"'IBM Plex Mono',monospace",transition:"all .2s",
                    boxShadow:sel===t.id?`0 -2px 8px ${t.color}22`:"none"}}>
                  {t.emoji} {t.name}
                </button>
              ))}
            </div>
      ...
      <div style={{position:"sticky",top:0,zIndex:100,background:C.bg+"f4",backdropFilter:"blur(16px)",borderBottom:`1px solid ${C.border}`,padding:"9px 28px",display:"flex",alignItems:"center",gap:14}}>
        <div style={{width:8,height:8,borderRadius:"50%",background:opt.color,boxShadow:`0 0 8px ${opt.color}`,flexShrink:0}}/>
        <div style={{fontSize:14,fontWeight:700,color:opt.color,fontFamily:"'Courier New',monospace",letterSpacing:1}}>{opt.emoji} {opt.name}</div>
