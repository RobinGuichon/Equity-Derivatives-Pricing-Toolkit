...
// q = continuous dividend yield (Merton 1973)
...
// ─── PRICERS — drift = (r - q - 0.5σ²)dt partout ───────────────────────────
...
function binomBermudan(S,K,T,r,q,σ,N,put,nDates){
  ...
  for(let n=N-1;n>=0;n--){
    ...
    for(let i=0;i<=n;i++){const Sn=S*Math.pow(u,n-2*i);const cont=disc*(p*V[i]+(1-p)*V[i+1]);V[i]=canEx?Math.max(Math.max(put?K-Sn:Sn-K,0),cont):cont;}
  }
  return V[0];
}
...
function parisianMC(S0,K,T,r,q,σ,H,isDown,isOut,window,put,Z){
  ...
  for(const path of Z){
    ...
    for(const z of path){S*=Math.exp(mu+v*z);if(isDown?S<=H:S>=H){cons++;if(cons>=req){hit=true;break;}}else{cons=0;}}
...
function shoutMC(S0,K,T,r,q,σ,put,Z){
  ...
  for(const path of Z){
    let S=S0,floor=0;
    for(let j=0;j<nS;j++){
      S*=Math.exp(mu+v*path[j]);const tRem=T-(j+1)*dt;
      if(floor===0&&(put?S<K:S>K)&&tRem>dt){const atm=bs(S,S,tRem,r,q,σ,put);if((put?K-S:S-K)>=atm)floor=put?K-S:S-K;}
    }
    tot+=Math.max(put?Math.max(K-S,0):Math.max(S-K,0),floor);
  }
  return disc*tot/Z.length;
}
...
function margrabeP(S1,S2,T,σ1,σ2,rho){
...
// fwdStart: discount by e^(-q*t0) since stock pays dividend before option starts
function fwdStart(S0,alpha,T,t0,r,q,σ,put){return S0*Math.exp(-q*t0)*bs(1,alpha,T-t0,r,q,σ,put);}
// Quanto: q_adj = ρ·σ·σFX adjusts the dividend yield
...
function autocallMC(S0,barrierPct,kiPct,couponRate,nObs,T,r,q,σ,nPaths,seed=42){
  return autocallFull(S0,barrierPct,kiPct,couponRate,nObs,T,r,q,σ,nPaths,seed).price;
}

// MC Path generators for visualization
function genMCPathsData(S0,T,r,q,σ,numPaths=100,numSteps=60,seed=42){
  const rand=lcg(seed),dt=T/numSteps,mu=(r-q-.5*σ*σ)*dt,v=σ*Math.sqrt(dt);
  ...
  for(let p=0;p<numPaths;p++){let S=S0;data[0][`path${p+1}`]=S;avgPath[0]+=S;for(let step=1;step<=numSteps;step++){S*=Math.exp(mu+v*bm(rand(),rand()));data[step][`path${p+1}`]=S;avgPath[step]+=S;}}
...
function genMultiMCPathsData(S1,S2,T,r,q,σ1,σ2,rho,numPaths=20,numSteps=60,seed=42){
  const rand=lcg(seed),dt=T/numSteps,mu1=(r-q-.5*σ1*σ1)*dt,v1=σ1*Math.sqrt(dt),mu2=(r-q-.5*σ2*σ2)*dt,v2=σ2*Math.sqrt(dt),sqRho=Math.sqrt(Math.max(1-rho*rho,0));
  const data=Array.from({length:numSteps+1},(_,step)=>({step}));
  const pathKeys1=[],pathKeys2=[];
  for(let p=0;p<numPaths;p++){
    const k1=`A${p+1}`,k2=`B${p+1}`;pathKeys1.push(k1);pathKeys2.push(k2);
    let A=S1,B=S2;data[0][k1]=A;data[0][k2]=B;
    for(let step=1;step<=numSteps;step++){const z1=bm(rand(),rand()),z2=bm(rand(),rand()),z2c=rho*z1+sqRho*z2;A*=Math.exp(mu1+v1*z1);B*=Math.exp(mu2+v2*z2c);data[step][k1]=A;data[step][k2]=B;}
  }
...
function Sl({label,value,min,max,step,unit,onChange,accent="#00e5ff"}){
  ...
  React.useEffect(()=>{if(parseFloat(value)!==parseFloat(localVal))setLocalVal(String(value));},[value]);
  const handleInputChange=(e)=>{setLocalVal(e.target.value);const parsed=parseFloat(e.target.value);if(!isNaN(parsed))onChange(parsed);};
  return(
    <div style={{marginBottom:11}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
        <span style={{color:C.muted,fontSize:10}}>{label}</span>
        <div style={{display:"flex",alignItems:"center"}}>
          <input type="text" value={localVal} onChange={handleInputChange} onBlur={()=>setLocalVal(String(value))}
            style={{background:C.bg,border:`1px solid ${C.border}`,color:C.bright,fontSize:10,fontWeight:700,width:46,padding:"3px 4px",borderRadius:4,textAlign:"right",fontFamily:"'IBM Plex Mono',monospace",marginRight:6,transition:"border 0.2s"}}
            onFocus={e=>e.target.style.border=`1px solid ${accent}`} onMouseLeave={e=>{if(document.activeElement!==e.target)e.target.style.border=`1px solid ${C.border}`;}}/>
...
function ChartBox({title,children,accent}){return<Pnl accent={accent} style={{marginTop:0}}><Lbl>{title}</Lbl>{children}</Pnl>;}
...
function GreeksGrid({data,refs=[],order="1"}){
  ...
  return(
    <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:11}}>
      ...
      {id!=="strategy"&&<><div style={{borderTop:`1px solid ${C.border}`,margin:"10px 0 8px",opacity:.4}}/><Tog opts={[{v:1,l:"▲ Long"},{v:-1,l:"▼ Short"}]} val={p.dir||1} onChange={v=>sp("dir",v)}/></>}
    </div>
  );
}
...
function plinearStrategy(p,n=70){
  ...
  const Smin=Math.max(0,minK*.5),Smax=maxK*1.5;
  return Array.from({length:n},(_,i)=>{const S=Smin+(Smax-Smin)*i/(n-1);const payoff=p.legs.reduce((sum,l)=>sum+l.dir*l.qty*Math.max(l.type==='put'?l.K-S:S-l.K,0),0);return{S:+S.toFixed(1),payoff};});
}

const OPTS=[
  {id:"european",name:"European",sub:"Black-Scholes Model",color:"#00e5ff",cat:"Vanilla",emoji:"📐",closedForm:true,
   dp:{S0:100,K:100,T:1,r:.05,q:0,sigma:.20,put:false,dir:1},
  ...
  {id:"bermudan",name:"Bermudan",sub:"Discrete Binomial Tree",color:"#10b981",cat:"Vanilla",emoji:"🌴",
   dp:{S0:100,K:100,T:1,r:.05,q:0,sigma:.20,put:false,nDates:4,dir:1},
  ...
  {id:"barrier",name:"Barrier",sub:"Knock-Out / Knock-In",color:"#f97316",cat:"Exotic",emoji:"🚧",
   dp:{S0:100,K:100,T:1,r:.05,q:0,sigma:.20,H:80,isDown:true,isOut:true,put:false,dir:1},
  ...
  {id:"parisian",name:"Parisian",sub:"Excursion Window",color:"#c084fc",cat:"Exotic",emoji:"🗼",
   dp:{S0:100,K:100,T:1,r:.05,q:0,sigma:.20,H:80,isDown:true,isOut:true,window:10,put:false,dir:1},
  ...
  {id:"digital",name:"Digital",sub:"Cash / Asset-or-Nothing",color:"#e879f9",cat:"Exotic",emoji:"⚡",closedForm:true,
   dp:{S0:100,K:100,T:1,r:.05,q:0,sigma:.20,isCall:true,isCash:true,dir:1},
  ...
  {id:"lookback",name:"Lookback",sub:"Floating/Fixed Strike",color:"#fb7185",cat:"Exotic",emoji:"👁",
   dp:{S0:100,K:100,T:1,r:.05,q:0,sigma:.20,isFloat:true,put:false,dir:1},
  ...
  {id:"basket",name:"Basket",sub:"2 Correlated Assets",color:"#fbbf24",cat:"Multi-Asset",emoji:"🧺",
   dp:{S0:100,S2:100,K:100,T:1,r:.05,q:0,sigma:.20,sigma2:.25,rho:.5,w1:.5,put:false,dir:1},
  ...
  {id:"rainbow",name:"Rainbow",sub:"Best-of / Worst-of",color:"#f43f5e",cat:"Multi-Asset",emoji:"🌈",
   dp:{S0:100,S2:100,K:100,T:1,r:.05,q:0,sigma:.20,sigma2:.25,rho:.5,type:"best-call",dir:1},
  ...
  {id:"quanto",name:"Quanto",sub:"FX Hedged Option",color:"#ec4899",cat:"Multi-Asset",emoji:"💱",closedForm:true,
   dp:{S0:100,K:100,T:1,r:.05,q:0,sigma:.20,sigma2:.15,rho:.3,put:false,dir:1},
  ...
  {id:"chooser",name:"Chooser",sub:"Call/Put Choice at t₀",color:"#06b6d4",cat:"Other",emoji:"🎯",
   dp:{S0:100,K:100,T:1,t0:.5,r:.05,q:0,sigma:.20,dir:1},
  ...
  {id:"cliquet",name:"Cliquet",sub:"Periodic Returns Cap/Floor",color:"#84cc16",cat:"Other",emoji:"🔗",
   dp:{S0:100,T:2,r:.05,q:0,sigma:.20,cap:.10,floor:-.05,nPeriods:4,dir:1},
  ...
  {id:"power",name:"Power",sub:"Payoff = S^n − K",color:"#f59e0b",cat:"Other",emoji:"⚡",
   dp:{S0:100,K:10000,T:1,r:.05,q:0,sigma:.20,n:2,put:false,dir:1},
  ...
  {id:"exchange",name:"Exchange",sub:"max(S₂−S₁, 0) Margrabe",color:"#38bdf8",cat:"Other",emoji:"🔄",closedForm:true,
   dp:{S0:100,S2:110,T:1,r:.05,q:0,sigma:.20,sigma2:.25,rho:.3,dir:1},
   pf:(S,p)=>margrabeP(S,p.S2||110,p.T,p.sigma,p.sigma2||.25,p.rho||.3),
  ...
  {id:"fwdstart",name:"Forward Start",sub:"Deferred Strike α·S(t₀)",color:"#34d399",cat:"Other",emoji:"⏩",closedForm:true,
   dp:{S0:100,alpha:1.0,T:2,t0:.5,r:.05,q:0,sigma:.20,put:false,dir:1},
  ...
  {id:"autocall",name:"Autocall",sub:"Structured Product MC",color:"#fb923c",cat:"Structured",emoji:"🏛",
   dp:{S0:100,barrierPct:1,kiPct:.6,couponRate:.10,nObs:4,T:4,r:.03,q:0,sigma:.20,dir:1},
...
function ParamSliders({opt,p,sp}){
        ...
        {p.legs.map((leg,i)=>(
          <div key={leg.id} style={{background:"#111b2b",padding:"10px",borderRadius:8,marginBottom:10,border:`1px solid ${C.border}`}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
              ...
              <button onClick={()=>sp("legs",p.legs.filter(l=>l.id!==leg.id))} style={{background:"transparent",border:"none",color:"#ef4444",cursor:"pointer",fontSize:10}}>✕</button>
            </div>
            ...
            <Sl label="Qty" value={leg.qty} min={1} max={10} step={1} unit="" accent="#fbbf24" onChange={v=>sp("legs",p.legs.map(l=>l.id===leg.id?{...l,qty:v}:l))}/>
          </div>
        ))}
        <button onClick={()=>sp("legs",[...p.legs,{id:Date.now(),type:'call',dir:1,K:p.S0,sigma:.20,qty:1}])} style={{width:"100%",padding:"8px 0",background:color+"11",border:`1px dashed ${color}`,color,borderRadius:7,cursor:"pointer",fontSize:10,fontFamily:"'IBM Plex Mono',monospace"}}>+ ADD LEG</button>
      </div>
    );
  }
  ...
  return(
    <div>
      ...
      {id==="quanto"&&<><Sl label="FX Vol σ_FX" value={((p.sigma2||.15)*100).toFixed(0)} min={5} max={80} step={1} unit="%" accent="#ec4899" onChange={v=>sp("sigma2",v/100)}/><Sl label="Correl ρ" value={((p.rho||.3)*100).toFixed(0)} min={-99} max={99} step={1} unit="%" accent="#fbbf24" onChange={v=>sp("rho",v/100)}/></>}
...
function AutocallExtraCharts({p,color}){
  ...
  const callProbData=mc.callProbs.map((prob,i)=>({obs:`Obs ${i+1}`,prob:+(prob*100).toFixed(1)}));
  ...
  return(
    <>
      ...
      <ChartBox title="Recall Probability per Observation" accent={color}>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={callProbData} margin={{top:4,right:8,left:0,bottom:0}}>
            <CartesianGrid stroke={C.border} strokeDasharray="3 3" vertical={false}/>
            <XAxis dataKey="obs" tick={{fill:C.muted,fontSize:9}} tickLine={false} axisLine={false}/>
            <YAxis tick={{fill:C.muted,fontSize:9}} tickLine={false} axisLine={false} width={34} tickFormatter={v=>`${v}%`}/>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={histData} margin={{top:4,right:8,left:0,bottom:0}}>
            ...
            <Bar dataKey="pct" radius={[4,4,0,0]}>
              {histData.map((e,i)=>{const mid=Number(e.range.split("–")[0])+5;const c=mid<p.kiPct*100?"#ef4444":mid>100?color:"#34d399";return<Cell key={i} fill={c} fillOpacity={1}/>;} )}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
...
function AsianExtraCharts({p,color}){
  ...
  return(
    <ChartBox title="Asian vs European by Volatility" accent={color}>
      ...
      <div style={{fontSize:8,color:C.muted,marginTop:6}}>Asian ≤ European always (averaging reduces variance)</div>
    </ChartBox>
  );
}
function BarrierExtraCharts({p,color}){
  ...
  return(
    <ChartBox title={`KO Price vs Barrier Level (${p.isDown?"Down":"Up"})`} accent={color}>
      <ResponsiveContainer width="100%" height={200}>
...
function MCPathsChart({S0,T,r,q,sigma,color,title="Monte Carlo Paths"}){
    ...
    <ChartBox title={title} accent={color}>
      <ResponsiveContainer width="100%" height={360}>
        <LineChart data={data} margin={{top:2,right:8,left:0,bottom:0}}>
          ...
          <Tooltip cursor={{stroke:"#ffffff22",strokeWidth:1}} content={({active,payload,label})=>{if(active&&payload){const avg=payload.find(p=>p.dataKey==='avg');if(!avg)return null;return<div style={{background:C.panel,border:`1px solid ${color}44`,padding:"8px 12px",borderRadius:7,fontFamily:"'IBM Plex Mono',monospace",fontSize:10}}><div style={{color:C.muted}}>Step {label}</div><div style={{color:"#ffffffaa",fontWeight:700}}>Avg: {avg.value.toFixed(2)}</div></div>;}return null;}}/>
          <ReferenceLine y={S0} stroke="#ffffff33" strokeDasharray="4 4"/>
          {pathKeys.map(key=><Line key={key} type="monotone" dataKey={key} stroke={color} strokeWidth={0.8} strokeOpacity={0.3} dot={false} activeDot={false} isAnimationActive={false}/>)}
          <Line type="monotone" dataKey="avg" stroke="#ffffffbb" strokeWidth={2.5} dot={false} isAnimationActive={false}/>
        </LineChart>
      </ResponsiveContainer>
      <div style={{fontSize:8,color:C.muted,marginTop:4}}>100 paths simulated · drift = (r − q − ½σ²)dt</div>
    </ChartBox>
  );
}
function MultiMCPathsChart({S1,S2,T,r,q,sigma1,sigma2,rho,color1,color2="#a78bfa"}){
  ...
  return(
    <ChartBox title="Correlated Multi-Asset Paths" accent={color1}>
      <ResponsiveContainer width="100%" height={220}>
        <LineChart data={data} margin={{top:2,right:8,left:0,bottom:0}}>
          <CartesianGrid stroke={C.border} strokeDasharray="3 3"/>
          <XAxis dataKey="step" type="number" domain={['dataMin','dataMax']} tick={{fill:C.muted,fontSize:9}} tickLine={false} axisLine={{stroke:C.border}} tickFormatter={v=>(v*T/60).toFixed(2)+"y"}/>
          <YAxis tick={{fill:C.muted,fontSize:9}} tickLine={false} axisLine={false} width={40}/>
          <ReferenceLine y={S1} stroke={color1} strokeOpacity={0.4} strokeDasharray="4 4"/>
          <ReferenceLine y={S2} stroke={color2} strokeOpacity={0.4} strokeDasharray="4 4"/>
          {pathKeys1.map(k=><Line key={k} type="monotone" dataKey={k} stroke={color1} strokeWidth={1} strokeOpacity={0.5} dot={false} activeDot={false} isAnimationActive={false}/>)}
          {pathKeys2.map(k=><Line key={k} type="monotone" dataKey={k} stroke={color2} strokeWidth={1} strokeOpacity={0.5} dot={false} activeDot={false} isAnimationActive={false}/>)}
        </LineChart>
      </ResponsiveContainer>
      <div style={{fontSize:8,color:C.muted,marginTop:6,display:"flex",gap:14}}>
        <span><span style={{color:color1}}>■</span> Asset 1</span>
        <span><span style={{color:color2}}>■</span> Asset 2</span>
...
function PricerPage({opt,p,sp}){
  ...
  const dir=cp.dir||1;
  const price=opt.pf(cp.S0,cp,Z1,Z2)*dir;
  const color=opt.color;
  const combinedData=React.useMemo(()=>{
    const base=opt.payoff(cp);
    return base.map(d=>({...d,price:+(opt.pf(d.S,cp,Z1,Z2)*dir).toFixed(3),payoff:+(d.payoff*dir).toFixed(3)}));
  },[cp,opt,Z1,Z2]);
  ...
  if(opt.id==="european"||opt.id==="american"||opt.id==="strategy"){
    ...
    if(opt.id==="strategy"){cp.legs.forEach(l=>{const lg=bsGreeks(cp.S0,l.K,cp.T,cp.r,cp.q||0,l.sigma,l.type==='put');const m=l.dir*l.qty;g.delta+=lg.delta*m;g.gamma+=lg.gamma*m;g.theta+=lg.theta*m;g.vega+=lg.vega*m;g.rho+=lg.rho*m;g.vanna+=lg.vanna*m;g.vomma+=lg.vomma*m;});}
    else{const raw=bsGreeks(cp.S0,cp.K,cp.T,cp.r,cp.q||0,cp.sigma,cp.put);
      g={delta:raw.delta*dir,gamma:raw.gamma*dir,theta:raw.theta*dir,vega:raw.vega*dir,rho:raw.rho*dir,vanna:raw.vanna*dir,vomma:raw.vomma*dir};}
    ...
    if(opt.id==="american"){const eu=bs(cp.S0,cp.K,cp.T,cp.r,cp.q||0,cp.sigma,cp.put);kpis=[{l:"American",v:price.toFixed(4),c:color},{l:"European",v:eu.toFixed(4),c:C.muted},{l:"Early Ex.",v:Math.max(price-eu,0).toFixed(4),c:"#fb923c"},...kpis.slice(0,4)];}
  }
  return(
    <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
      <div style={{flex:"0 0 200px"}}>
        <Pnl style={{position:"sticky",top:72}} accent={color}>
          ...
          {!isFast&&<div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,marginTop:4}}>
            <button onClick={run} disabled={busy} style={{width:"100%",padding:"9px 0",background:"transparent",border:`1px solid ${busy?"#f59e0b":color}`,color:busy?"#f59e0b":color,borderRadius:7,cursor:busy?"not-allowed":"pointer",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,letterSpacing:2,textTransform:"uppercase",transition:"all .2s"}}>{busy?"⏳ Computing...":"▶ Recalculate"}</button>
          </div>}
        </Pnl>
      </div>
      <div style={{flex:1,minWidth:260,display:"flex",flexDirection:"column",gap:12,opacity:busy?.4:1,transition:"opacity 0.2s"}}>
        <Pnl accent={color}>
          <Lbl>Theoretical Value</Lbl>
          <div style={{display:"flex",alignItems:"baseline",gap:10}}>
            <div style={{fontSize:46,fontWeight:700,color,fontFamily:"'Courier New',monospace",lineHeight:1,letterSpacing:1}}>{price!=null?(dir===-1?"-":"")+Math.abs(price).toFixed(3):"-"}</div>
            {dir===-1&&<span style={{fontSize:9,padding:"3px 8px",borderRadius:4,background:"#ef444422",color:"#ef4444",border:"1px solid #ef444433",letterSpacing:2,alignSelf:"center"}}>SHORT · REÇU</span>}
            {opt.id==="autocall"&&<span style={{fontSize:16,color:C.muted}}>% of Notional</span>}
          </div>
        </Pnl>
        {kpis.length>0&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(95px,1fr))",gap:8}}>{kpis.map(k=><Kpi key={k.l} label={k.l} value={k.v} color={k.c}/>)}</div>}
        <Pnl accent={color}>
          ...
          <div style={{display:"flex",gap:16,marginTop:8,fontSize:9,color:C.muted,justifyContent:"center",letterSpacing:1}}>
            <span><span style={{color:"#64748b",fontWeight:700}}>---</span> PAYOFF AT EXPIRY</span>
            <span><span style={{color,fontWeight:700}}>—</span> CURRENT PRICE</span>
          </div>
        </Pnl>
        {["autocall","barrier","asian","parisian","lookback","shout","chooser","cliquet","power"].includes(opt.id)&&<MCPathsChart S0={cp.S0} T={cp.T} r={cp.r} q={cp.q||0} sigma={cp.sigma} color={color}/>}
        {["basket","rainbow"].includes(opt.id)&&<MultiMCPathsChart S1={cp.S0} S2={cp.S2||100} T={cp.T} r={cp.r} q={cp.q||0} sigma1={cp.sigma} sigma2={cp.sigma2||.25} rho={cp.rho||.5} color1={color}/>}
...
function GreeksPage({opt,p,sp}){
  ...
  React.useEffect(()=>{if(isFast){const t=setTimeout(run,10);return()=>clearTimeout(t);}},[p,isFast]);
      ...
      <div style={{flex:"0 0 200px"}}>
        <Pnl style={{position:"sticky",top:72}} accent={opt.color}>
          ...
          <div style={{borderTop:`1px solid ${C.border}`,paddingTop:12,marginTop:4}}>
            ...
            {isFast?<div style={{width:"100%",padding:"9px 0",background:"transparent",border:`1px solid ${opt.color}`,color:opt.color,borderRadius:7,fontFamily:"'IBM Plex Mono',monospace",fontSize:10,letterSpacing:2,textTransform:"uppercase",textAlign:"center"}}>⚡ Real-Time</div>
            :<button onClick={run} disabled={busy} style={{width:"100%",padding:"9px 0",background:"transparent",border:`1px solid ${busy?"#f59e0b":opt.color}`,color:busy?"#f59e0b":opt.color,borderRadius:7,cursor:busy?"not-allowed":"pointer",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,letterSpacing:2,textTransform:"uppercase",transition:"all .2s"}}>{busy?"⏳ Computing...":"▶ Recalculate"}</button>}
            <div style={{marginTop:8,fontSize:8,color:C.muted,textAlign:"center",lineHeight:1.5}}>24 pts · {isFast?"Closed-Form":"2,000 MC"}<br/>Central Finite Diff</div>
          </div>
        </Pnl>
      </div>
      <div style={{flex:1,minWidth:0,opacity:busy&&!isFast?.4:1,transition:"opacity 0.2s"}}>
        {data&&<>
          <GreeksGrid data={data} refs={refs} order={order}/>
          {order==="2"&&<div style={{marginTop:14,padding:"11px 16px",background:C.panel,borderRadius:8,border:`1px solid ${C.border}`,fontSize:9,color:C.muted,display:"flex",gap:18,flexWrap:"wrap"}}>
            {[{n:"Vanna",f:"∂Δ/∂σ",c:"#f472b6"},{n:"Vomma",f:"∂²V/∂σ²",c:"#4ade80"},{n:"Charm",f:"∂Δ/∂t",c:"#fb923c"},{n:"Speed",f:"∂Γ/∂S",c:"#c084fc"},{n:"Veta",f:"∂ν/∂t",c:"#22d3ee"}].map(({n,f,c})=><span key={n}><span style={{color:c,fontWeight:700}}>{n}</span> = {f}</span>)}
          </div>}
        </>}
      </div>
    </div>
...
function generate3DVolSurface(S0,baseVol,skewFactor,smileFactor){
  ...
  for(let t=.1;t<=2.0;t+=.1)maturities.push(t.toFixed(1));
  for(let i=0;i<maturities.length;i++){const T=Number(maturities[i]),zRow=[];for(let j=0;j<strikes.length;j++){const K=Number(strikes[j]),m=Math.log(K/S0),skew=skewFactor/Math.sqrt(T),smile=smileFactor/Math.sqrt(T);zRow.push(Math.max(.01,baseVol+skew*m+smile*m*m)*100);}zData.push(zRow);}
  return{strikes,maturities,zData};
}
function VolSurfacePage({color}){
  const[S0,setS0]=React.useState(100);
  const[baseVol,setBaseVol]=React.useState(.20);
  const[skewFactor,setSkewFactor]=React.useState(-.15);
  const[smileFactor,setSmileFactor]=React.useState(.08);
    ...
    <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
      <div style={{flex:"0 0 200px"}}>
        <Pnl style={{position:"sticky",top:72}} accent={color}>
          ...
          <Sl label="Skew (Slope)" value={skewFactor} min={-.50} max={.50} step={.01} unit="" accent="#38bdf8" onChange={setSkewFactor}/>
          <Sl label="Smile (Convexity)" value={smileFactor} min={0} max={.50} step={.01} unit="" accent="#f472b6" onChange={setSmileFactor}/>
          <div style={{marginTop:16,fontSize:9,color:C.muted,lineHeight:1.6}}>Drag to rotate the 3D surface.</div>
        </Pnl>
      </div>
      <div style={{flex:1,minWidth:0}}>
        <Pnl accent={color}>
          <Lbl>Implied Volatility Surface</Lbl>
          <div style={{width:'100%',height:500}}>
            <Plot data={[{z:zData,x:strikes,y:maturities,type:'surface',colorscale:[[0,'#00e5ff'],[.5,'#a78bfa'],[1,'#f472b6']],showscale:false}]}
              layout={{autosize:true,margin:{l:0,r:0,b:10,t:10},paper_bgcolor:'transparent',plot_bgcolor:'transparent',font:{color:C.muted,family:'"IBM Plex Mono",monospace',size:10},scene:{xaxis:{title:'Strike K',gridcolor:C.border,zerolinecolor:C.border,showbackground:false},yaxis:{title:'Maturity T (y)',gridcolor:C.border,zerolinecolor:C.border,showbackground:false},zaxis:{title:'IV (%)',gridcolor:C.border,zerolinecolor:C.border,showbackground:false},camera:{eye:{x:1.5,y:1.5,z:.6}}}}}
  ...
  return(
    <div>
      <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:20,flexWrap:"wrap"}}>
        ...
        <span style={{fontSize:8,padding:"2px 8px",borderRadius:4,background:opt.closedForm?"#00e5ff11":"#f59e0b11",color:opt.closedForm?"#00e5ff":"#f59e0b",border:`1px solid ${opt.closedForm?"#00e5ff33":"#f59e0b33"}`,letterSpacing:2}}>{opt.closedForm?"CLOSED-FORM":"MONTE CARLO"}</span>
      </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'IBM Plex Mono',monospace",boxSizing:"border-box"}}>
      <div style={{background:"linear-gradient(180deg,#070f1f 0%,#030712 100%)",borderBottom:`1px solid ${C.border}`,padding:"24px 28px 0"}}>
        <div style={{maxWidth:1480,margin:"0 auto"}}>
          <div style={{textAlign:"center",paddingBottom:18}}>
            ...
            <h1 style={{margin:"0 0 10px",fontFamily:"'Courier New',monospace",fontSize:"clamp(26px,4vw,44px)",fontWeight:700,letterSpacing:4,lineHeight:1,background:"linear-gradient(110deg,#d4e4f7 0%,#00e5ff 40%,#f59e0b 75%,#fb923c 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>OPTIONS  LAB</h1>
            <div style={{fontSize:10,color:C.muted,letterSpacing:2,marginBottom:18}}>{OPTS.length} Option Types &nbsp;·&nbsp; 6 First-Order Greeks &nbsp;·&nbsp; 5 Second-Order Greeks &nbsp;·&nbsp; Merton q ✓</div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"center"}}>
              {OPTS.map(t=>(
                <button key={t.id} onClick={()=>setSel(t.id)} style={{padding:"6px 13px",borderRadius:"8px 8px 0 0",border:`1px solid ${sel===t.id?t.color:C.border}`,borderBottom:"none",background:sel===t.id?t.color+"22":C.panel,color:sel===t.id?t.color:C.muted,cursor:"pointer",fontSize:10,letterSpacing:.5,fontFamily:"'IBM Plex Mono',monospace",transition:"all .2s",boxShadow:sel===t.id?`0 -2px 8px ${t.color}22`:"none"}}>
                  {t.emoji} {t.name}
                </button>
              ))}
            </div>
      ...
      <div style={{position:"sticky",top:0,zIndex:100,background:C.bg+"f4",backdropFilter:"blur(16px)",borderBottom:`1px solid ${C.border}`,padding:"9px 28px",display:"flex",alignItems:"center",gap:14}}>
        ...
        <div style={{marginLeft:"auto",fontSize:9,color:C.dim,letterSpacing:2}}>BS · MC · MERTON q · {new Date().getFullYear()}</div>
      </div>
      <div style={{display:"flex",gap:16,padding:"18px 20px 60px",maxWidth:1480,margin:"0 auto"}}>
        <Sidebar sel={sel} onSel={setSel}/>
        <div style={{flex:1,minWidth:0}}><OptionPage key={sel} opt={opt}/></div>
      </div>
    </div>
  );
}
