...
function PricerPage({opt,p,sp}){
  ...
  const combinedData=React.useMemo(()=>{
    ...
    return basePayoff.map(d=>{const pr=opt.pf(d.S,cp,Z1,Z2);return{...d,price:+pr.toFixed(3)};});
  },[cp,opt,Z1,Z2]);
  ...
  if(opt.id==="european"||opt.id==="american"||opt.id==="strategy"){
    ...
    if(opt.id==="strategy"){
      cp.legs.forEach(l=>{const legG=bsGreeks(cp.S0,l.K,cp.T,cp.r,cp.q||0,l.sigma,l.type==='put');const mult=l.dir*l.qty;g.delta+=legG.delta*mult;g.gamma+=legG.gamma*mult;g.theta+=legG.theta*mult;g.vega+=legG.vega*mult;g.rho+=legG.rho*mult;g.vanna+=legG.vanna*mult;g.vomma+=legG.vomma*mult;});
    }else{g=bsGreeks(cp.S0,cp.K,cp.T,cp.r,cp.q||0,cp.sigma,cp.put);}
    ...
    if(opt.id==="american"){const eu=bs(cp.S0,cp.K,cp.T,cp.r,cp.q||0,cp.sigma,cp.put);kpis=[{l:"American",v:price.toFixed(4),c:color},{l:"European",v:eu.toFixed(4),c:C.muted},{l:"Early Exer. Prem.",v:Math.max(price-eu,0).toFixed(4),c:"#fb923c"},...kpis.slice(0,4)];}
  }
  return(
